...
- ファイルサイズの「MB」は内部で 1 MiB = 1,048,576 bytes として計算します。Discordなどのアップロード上限を超えにくくするため、現在は目標サイズから一律6%の安全マージンを取り、音声とMP4コンテナ分も見込んで映像ビットレートを決めます。例えば500 MBを指定した場合、圧縮結果はおおむね470 MiB付近を目指します。
- 固定ビットレートの2回エンコードでも、映像内容やMP4コンテナの都合で結果のサイズを厳密に保証するものではありません。安全マージンの適切な量も動画の長さや内容によって変わるため、現時点では「目標以下になりやすいこと」を優先した固定値です。
